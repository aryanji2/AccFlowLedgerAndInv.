...
export default function PayableCheques() {
  ...
  const cleanupOldPaidCheques = async () => {
    if (!selectedFirm?.id) return;

    try {
      const { data: paidCheques, error } = await supabase
        .from('payable_cheques')
        .select('id, paid_date')
        .eq('firm_id', selectedFirm.id)
        .eq('status', 'paid')
      ...
      if (paidCheques && paidCheques.length > 10) {
        const idsToDelete = paidCheques.slice(10).map(c => c.id);
        ...
        console.log(`Deleted ${idsToDelete.length} old paid cheques`);
      }
    } catch (error) {
      console.error('Error cleaning up old paid cheques:', error);
    }
  };
  ...
  const handleStatusUpdate = async (chequeId: string, newStatus: 'paid' | 'cancelled') => {
    ...
    try {
      ...
      if (newStatus === 'paid') {
        await cleanupOldPaidCheques(); // ✅ Cleanup after mark as paid
      }
